...
def login_view(request):
    if request.method == 'GET':
        ...
        try:
            ip = get('https://api.ipify.org').text
            # print(ip)
            send_url =('https://geo.ipify.org/api/v1?apiKey=at_vbEmmp5PuMaaA6UNqrXGrhKd6feud&ipAddress='+ip)
            print(send_url)
            # os = request.user_agent.os.family,
            # browser = request.user_agent.browser.family
            # browserversion = request.user_agent.os.version
            # send_url = 'http://api.ipstack.com/check?access_key=38232ac57f8eef77a92b96b3fcb97ff7'
            r = requests.get(send_url)
            print(r)
            message = r.content
